...
namespace MLAgents
{
    ...
    public class RpcCommunicator : ICommunicator
    {
        ...
        Dictionary<string, List<Tuple<Agent, AgentAction>>> m_LastActionsReceived =
            new Dictionary<string, List<Tuple<Agent, AgentAction>>>();
...
# if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX
        ...
        void SendBatchedMessageHelper()
        {
            ...
            foreach (var brainName in rlInput.AgentActions.Keys)
            {
                ...
                var agentActionTup = new List<Tuple<Agent, AgentAction>>(numAgents);
                m_LastActionsReceived[brainName] = agentActionTup;
                for (var i = 0; i < numAgents; i++)
                {
                    var agent = m_CurrentAgents[brainName][i];
                    agentActionTup.Add(new Tuple<Agent, AgentAction>(agent, agentActions[i])); 
                }
            }
        }

        public List<Tuple<Agent, AgentAction>> GetActions(string key)
        {
